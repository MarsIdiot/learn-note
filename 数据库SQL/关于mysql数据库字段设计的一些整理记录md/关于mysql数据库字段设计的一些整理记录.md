...
- 如果字段是not null的话，必须插入一个值，若不插入值的话，就会调用默认值。 如果这两个都没有的话，就会报错了。
...
**NULL表示“此字段值为空”，而”表示“此字段暂未设置值”，即空值**
...
![](D:\ztl\笔记\数据库SQL\关于mysql数据库字段设计的一些整理记录md\pictures\QQ截图20181217174648.png)

​	datetime:

![](D:\ztl\笔记\数据库SQL\关于mysql数据库字段设计的一些整理记录md\pictures\QQ截图20181217174625.png)
...
![QQ截图20181210142051](D:\ztl\笔记\数据库SQL\关于mysql数据库字段设计的一些整理记录md\pictures\QQ截图20181210142051.png)
...
![QQ截图20181210142603](D:\ztl\笔记\数据库SQL\关于mysql数据库字段设计的一些整理记录md\pictures\QQ截图20181210142603.png)
...
![20181210115844](D:\ztl\笔记\数据库SQL\关于mysql数据库字段设计的一些整理记录md\pictures\QQ截图20181210115844.png)
...
2、在某字段需要频繁改写的时候，用char会好一些（因为varchar每次存储都要有额外的计算，得到长度等工作，如果一个非常频繁改变的，那就要有很多的精力用于计算，而这些对于char来说是不需要的。）
...
![QQ截图20181210164217.](D:\ztl\笔记\数据库SQL\关于mysql数据库字段设计的一些整理记录md\pictures\QQ截图20181210164217.png)
...
```ini
sql_mode=NO_ENGINE_SUBSTITUTION,STRICT_TRANS_TABLES3、常用场景
```
